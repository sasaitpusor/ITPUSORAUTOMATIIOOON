/**
 * Încărcarea și derivarea configurației de client.
 *
 * Tot ce e specific unui client trăiește în config/clients/<id>.json.
 * Toate listele derivate (tag-uri, opțiuni de custom field, celule de conținut)
 * se calculează aici, ca să nu existe liste hardcodate nici în scripturi,
 * nici în workflow-urile n8n.
 */
...
/** Rezolvă o cale dot-notation în config, ex. "client.agency_name". */
...
/** Tag-urile de sistem: consimțământ, opt-out, stare de flux, sursă, ciclu de viață. */
 ...
 * (filtru de audiență, se aplică la selecția contactelor), `content_varies_by`
 * spune pe ce axe se scrie text DIFERIT. Fără separarea asta, cele 5 axe cerute
 * pentru ofertele personalizate produc peste 10.000 de texte de revizuit uman.
 */
export function deriveContentCells(config) {
  const cells = [];
  for (const [flowId, flow] of Object.entries(config.flows)) {
    if (!flow.enabled) continue;
    // Textul variază doar pe content_varies_by. Celelalte axe din segmented_by
    // filtrează audiența, deci nu multiplică numărul de variante de scris.
